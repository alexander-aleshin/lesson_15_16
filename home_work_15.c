...
int coins[2][5] = {
       {50, 10, 5, 2, 1},  // номиналы используемых монет в порядке убывания
...
void Greedy_Kopeks(int i) {
    ...
    if (cur_kopeks > coins[0][i]) {   // если номинал монеты меньше оставшегося значения
    ...
    Greedy_Kopeks(i + 1);  // рекурсивно заполняем монетами меньшего номнала
}
